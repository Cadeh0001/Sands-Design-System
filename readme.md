...
- **Ship-state rule.** Client-facing output contains zero meta-commentary about the document's own completeness — no "to be finalized", "pending confirmation", "model to follow", "subject to reconciliation". Unknown values in tables render as em dashes, never `[TBD]`. Bracketed placeholders are working-draft furniture: resolve or cut every one before the deck ships.
- **Key Takeaway / Read bars: two sentences max, one idea.** Never restate math already visible in the table above, never promise future deliverables, never note document status.
- **Methodology qualifiers go in an asterisk footnote,** not the takeaway bar — a small muted line beside the footer (e.g. "*Going-in cap based on Y1 rents, not current").
- **One headline cap per deal,** anchored to the underwriting-year rent and asterisk-footnoted. Do not recompute headline caps as rents escalate mid-deck.
- **Debt and net-proceeds slides are opt-in.** Default BOV carries financing as a single sentence in the current-situation slide (loan, cash, notes, closing costs). Use `Debt-Facility-Detail.html` and `Net-Proceeds-Waterfall.html` only when the client asks for them.
- **Comp tables — fixed column spec.** On-market: tenant / address, price, $/SF, cap, rent/SF, NOI, years remaining, lease structure, land size, year built, days on market, 3-mi population, 3-mi median HHI, guarantor. Sold adds sold date after the address. Same order every deck, every asset class — add columns only when the client asks.
- **Comp table conventions:** subject rows pinned to the top with the orange tint and the tenant's own name in the guarantor column; average/total row bold at 26px over a 3px navy rule with the navy tint behind it; em dashes for fields the survey does not carry. Order on-market comps by days on market ascending; sold comps most recent first. State the comp count in the average row label ("Average (6 sales)") and re-derive every average when a row is added or dropped — including the ex-outlier figure if the read bar cites one.
- **Pricing analysis — fixed structure.** Three scenarios at .25% spreads, tightest cap first, one matrix per package with a labelled eyebrow above each; NOI and basis columns to the left of the scenario columns; portfolio/total row bold over a 3px navy rule. Per-property lines in a portfolio matrix are allocated pro-rata by contract rent and labelled as analysis-only, never as transactional value. State the recommended range in the subhead in orange, matching the span of the matrix columns.
- **Footer consistency:** every content slide ends with the same footer — "Sands Investment Group | Confidential · sandsig.com" at left, SIG icon mark at right (42px; white variant on navy grounds). No slide ships without it.
...
- `slides/` additions from the Shriber BOV: why-SIG divider, by the numbers, track record (per product type), marketing plan + timeline, team (three profiles), confidentiality agreement, and two on-request layouts — net proceeds waterfall, debt / facility detail
- `assets/team/` — advisor headshots pre-framed to the 300×348 profile slot
